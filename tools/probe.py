...
CASES = [
    ...
    ("ident_ws_only",  ac(flight="        "), None, "flight is padding only — ident and op class?"),
...
def run_case(name, body, expect, why, mock):
    ...
    for _ in range(20):
        time.sleep(1)
        h = health()
        if h.get("fetch_ok", 0) + h.get("fetch_empty", 0) + h.get("fetch_fail", 0) > before:
            time.sleep(2.5)  # let the render settle before sampling
            h = health()
            break
